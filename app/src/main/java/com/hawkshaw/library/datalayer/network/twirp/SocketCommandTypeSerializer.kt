...
/**
 * Serializer for SocketCommandRequest.Type enum values
 */
object SocketCommandTypeSerializer : KSerializer<SocketCommandRequest.Type> {
    private const val TAG = "SocketCmdTypeSerializer" // Added for logging
    private val className = SocketCommandRequest.Type::class.qualifiedName
        ?: "com.hawkshaw.library.datalayer.models.SocketCommandRequest.Type"
    
    // Maps for fast lookup in both directions
    private val lookup = SocketCommandRequest.Type.entries.associateWith { it.name }
    private val revLookup = SocketCommandRequest.Type.entries.associateBy { it.name }

    init {
        Log.d(TAG, "[DEBUG] Initializing SocketCommandTypeSerializer. ClassName for descriptor: $className") // Added Logcat
    ...
    override fun serialize(encoder: Encoder, value: SocketCommandRequest.Type) {
    ...
    override fun deserialize(decoder: Decoder): SocketCommandRequest.Type {
        ...
        return if (commandType != null) {
            ...
            Log.w(TAG, "[DEBUG] deserialize: \"$decodedString\" not found in revLookup. Defaulting to SocketCommandRequest.Type.Unknown.") // Added Logcat
